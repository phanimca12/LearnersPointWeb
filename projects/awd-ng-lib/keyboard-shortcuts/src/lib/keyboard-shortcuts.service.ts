...
export class KeyboardShortcutsService implements OnDestroy {
  ...
  private isAllowed = (shortcut: ParsedShortcut) => {
    ...
    if (shortcut.allowIn.length) {
      return !difference(this._ignored, shortcut.allowIn).includes(
        target.nodeName
      );
    }
    ...
    return this._shortcuts
      ...
      .map((shortcut) =>
        Object.assign({}, shortcut, {
          predicates: any(
            identity,
            shortcut.predicates.map((predicates: any) =>
              allPass(predicates)(event)
            )
          ),
          event: event,
        })
      ...
      .reduce(
        (acc, shortcut) => (acc.priority > shortcut.priority ? acc : shortcut),
        {
          priority: 0,
        } as ParsedShortcut
      );
  ...
  constructor(
    ...
    private keyboardRegisterService: KeyboardRegisterDocumentService
  ) {
    this.addEventHandlers(this.document);

    this.keyboardRegisterService
      .getExternalWindows()
      .subscribe((externalWindows) => {
        externalWindows.forEach((externalWindow: ExternalWindow) => {
          this.addEventHandlers(
            externalWindow.externalWindow.document,
            externalWindow.windowData
          );
        });
      });
  }

  addEventHandlers(document: Document, documentData?: any): void {
    let clicks$ = fromEvent(document, 'click', { capture: true });

    let keyup$ = fromEvent(document, 'keyup');

    let keydown$ = fromEvent(document, 'keydown');

    let keydownCombo$ = keydown$.pipe(
    ...
    let keydownSequence$ = this.shortcuts$.pipe(
      ...
      scan(
        (acc: { events: any[]; command?: any; sequences: any[] }, arg: any) => {
          ...
          const result = sequences
            ...
            .filter(
              (sequences) => sequences.partialMatch || sequences.fullMatch
            );
      ...
      tap(
        (shortcut) =>
          !shortcut.preventDefault || shortcut.event.preventDefault()
      ),
      throttle((shortcut) => timer(shortcut.throttleTime)),
      tap((shortcut) => {
        shortcut.command({
    ...
    this.subscriptions.push(keydownCombo$.subscribe());
    this.subscriptions.push(keydownSequence$.subscribe());
  }
  ...
  private isSequence(shortcuts: string[]): boolean {
    return !shortcuts.some(
      (shortcut) => shortcut.includes('+') || shortcut.length === 1
    );
  }
  ...
  public remove(ids: string | string[]): KeyboardShortcutsService {
    ...
    // console.log('before: ')
    // console.log('429:', this._shortcuts);
    this._shortcuts = this._shortcuts.filter(
      (shortcut) => !ids.includes(shortcut.id)
    );
    this._sequences = this._sequences.filter(
      (shortcut) => !ids.includes(shortcut.id)
    );
    // console.log('after: ');
    // console.log('433:', this._shortcuts);
    setTimeout(() => {
      this._shortcutsSub.next([...this._shortcuts, ...this._sequences]);
    });
    return this;
  ...
  private parseCommand(
    command: ShortcutInput | ShortcutInput[]
  ): ParsedShortcut[] {
    const commands = Array.isArray(command) ? command : [command];
    return commands.map((command) => {
      const keys = Array.isArray(command.key) ? command.key : [command.key];
      const priority = Math.max(
        ...keys.map((key) => key.split(' ').filter(identity).length)
      );
      const predicates = keys.map((key) =>
        this.getKeys(key.split(' ').filter(identity))
      );
      ...
      return {
        ...
        key: keys,
        id: `${guid++}`,
        throttle: isNill(command.throttleTime)
          ? this.throttleTime
          : command.throttleTime,
        priority: priority,
